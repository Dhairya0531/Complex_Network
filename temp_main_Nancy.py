...
if clean_vals: ax1.set_ylim(0, max(clean_vals) * 1.2)
...
clean_vals = [v for v in values if not np.isnan(v)]
if clean_vals: ax2.set_ylim(0, max(clean_vals) * 1.2)
...
clean_vals = [v for v in values if not np.isnan(v)]
if clean_vals: ax3.set_ylim(0, max(clean_vals) * 1.2)
...
fig6.legend(handles, labels_leg, loc="center left", ncol=1, bbox_to_anchor=(1.0, 0.5), prop={"size": 18})
plt.tight_layout()
